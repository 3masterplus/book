<?php if (! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Tool extends MY_Controller{
		...
		//异步方式生成验证码
		function ajax_generate_a_captcha()
		{
			...
			//生成验证码
			$captcha = $this->captcha_lib->create_a_captcha($this->_ip_address);
			...
			return $this->my_lib->response($data, $message, $type, $result);
		}
	     ...
	     * @return array
	     */
	    function ajax_token_get($isEcho = true, $force_update = false)
	    {
	        $t            = time();
	        $new_generate = true;
	        $wx_data      = array();
	    ...
	    function update_wx_js_ticket($access_token_data, $recursive_time = 0)
	    {
	        ...
	        if (isset($wx_data['ticket']) && isset($wx_data['expires_in']) ){
	            ...
	            if ($recursive_time < 3) {
	                $re_wx_data = $this->ajax_token_get(false, true);
